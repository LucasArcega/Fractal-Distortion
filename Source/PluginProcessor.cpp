...
void FractalDistortionAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
    // Instancia a Tube engine
    //  Cria uma engine por canal (stereo = 2 canais, mono = 1 canal)
    distortionEngines.clear();
    distortionEngines.resize(getTotalNumOutputChannels());

    for (auto &engine : distortionEngines) {
...
void FractalDistortionAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
    ...
    for (int channel = 0; channel < buffer.getNumChannels(); ++channel) {
        ...
        auto &engine = this->distortionEngines[channel];
